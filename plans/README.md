...
| 058  | Media pause v2 — upgrade existing MediaPauseController to VoiceInk-grade (macOS perl MediaRemote bridge + enigo CGEvent key + resume delay + mute fallback; Windows session ledger + Store manifest capability) | P1 | M | — | IN PROGRESS — claimed Main 2026-08-21; macOS action layer landed first (MediaRemote command → NX media key → verified CoreAudio mute; live-diagnosed against a browser player that ignores all pause commands) |
