...
pub mod system {
    ...
    pub fn log_resources_before(operation: &str) {
        ...
        log::info!("💻 {}_BEFORE | CPU: {:.1}% | Memory: {}MB | Disk: {:.1}GB", 
            operation.to_uppercase(), resources.cpu_usage, resources.memory_usage_mb, resources.disk_available_gb);
    ...
    pub fn log_resources_after(operation: &str, duration_ms: u64) {
        ...
        log::info!("💻 {}_AFTER_{}ms | CPU: {:.1}% | Memory: {}MB | Disk: {:.1}GB", 
            operation.to_uppercase(), duration_ms, resources.cpu_usage, resources.memory_usage_mb, resources.disk_available_gb);
    }
            ...
            Err(error) => {
                ...
                if !context.is_empty() {
                    let ctx: Vec<(&str, &str)> = context.iter()
                        .map(|(k, v)| (k.as_str(), v.as_str()))
                        .collect();
                    log_with_context(log::Level::Debug, &format!("{} failure context", name), &ctx);
                }
                Err(error)
            }
        }
