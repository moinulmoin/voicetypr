...
{}",
    ...
    async fn make_request_with_retry(&self, request: &GeminiRequest) -> Result<GeminiResponse, AIError> {
    ...
    async fn make_single_request(&self, request: &GeminiRequest) -> Result<GeminiResponse, AIError> {
        let url = format!("{}/{}:generateContent", 
            self.base_url, 
            self.model
        );
...
impl AIProvider for GeminiProvider {
    async fn enhance_text(&self, request: AIEnhancementRequest) -> Result<AIEnhancementResponse, AIError> {
        ...
        let enhanced_text = gemini_response.candidates
            .first()
...
mod tests {
    ...
    fn test_provider_creation() {
        let result = GeminiProvider::new("".to_string(), "gemini-1.5-flash".to_string(), HashMap::new());
        assert!(result.is_err());
        
        let result = GeminiProvider::new("test_key_12345".to_string(), "invalid-model".to_string(), HashMap::new());
        assert!(result.is_err());
        
        let result = GeminiProvider::new("test_key_12345".to_string(), "gemini-2.5-flash-lite".to_string(), HashMap::new());
        assert!(result.is_ok());
    }
}